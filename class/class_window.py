...
import sys
sys.path.append('/home/sami/Documents/h3school/Computer/pajoheshi/pss-master/session/')
...
####        for i in sessions:
####            self.session_widget(self.SLW, i, files)
    ...
    def session_widget(self, w, text):
        itemN = QtWidgets.QListWidgetItem()
        widget = QtWidgets.QWidget()
        
        widgetButton = QtWidgets.QPushButton(text)
        widgetLayout = QtWidgets.QHBoxLayout()
      
        widgetLayout.addWidget(widgetButton)
        widgetLayout.addStretch()
        self.classes += [widgetButton]
...
if __name__ == "__main__":
    ...
    ui = Class_window(['hassan'])
    
    ui.show()
    ui.session_widget(ui.SLW, 'salam')
    ui.session_widget(ui.SLW, 'salam')
    sys.exit(app.exec_())
